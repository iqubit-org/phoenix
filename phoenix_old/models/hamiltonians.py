...
from phoenix_old.utils.passes import concatenate_subcircuits
...
class Hamiltonian(SparsePauliOp):
    def __init__(
        ...
        *,
        ...
        super().__init__(data, coeffs, ignore_pauli_phase=ignore_pauli_phase, copy=copy)
...
class HamiltonianModel:
    ...
    def group_paulis_and_coeffs(self) -> Dict[Tuple, Tuple[List[str], np.ndarray]]:
        ...
        from phoenix_old.primitive.grouping import group_paulis_and_coeffs
    ...
    def reconfigure(self) -> List[Union[BSF, Clifford2Q]]:
        ...
        """
        from phoenix_old.primitive.simplification import simplify_bsf
    ...
    def phoenix_reconfigure_holistic(self) -> List[List[Union[BSF, Clifford2Q]]]:
        from phoenix_old.holistic.simplification import simplify_bsf
        ...
        if cliffords_with_locals:  # because it might be an empty list
            post_cliffs_with_locals = [[cliff, local_bfs] if local_bfs.num_paulis > 0 else [cliff] for
                                       cliff, local_bfs
                                       in reversed(cliffords_with_locals)]
            config.extend([cliff for cliff, _ in cliffords_with_locals])  # pre-cliffs
            config.append(bsf_)
            config.extend([item for sublist in post_cliffs_with_locals for item in sublist])
        else:
            config.append(bsf_)
    ...
    def reconfigure_and_generate_circuit(self, by: str = 'cnot', order: int = 1) -> Circuit:
        ...
        def trotter_config(config: List[Union[BSF, Clifford2Q]], ord: int = 1, scale: float = 1) -> Circuit:
            ...
            from phoenix_old.primitive import utils
            if ord == 1:
    ...
    def phoenix_circuit_holistic(self) -> Circuit:
        from phoenix_old.primitive import utils
        from phoenix_old.primitive.utils import optimize_phoenix_circuit_by_qiskit

        config = self.phoenix_reconfigure_holistic()
        circ = utils.config_to_circuit(config)
        # TODO: add the following pass
        # circ = optimize_phoenix_circuit_by_qiskit(circ, optimization_level=2)  # Clifford2Q gate cancellation
    ...
    def phoenix_circuit(self, by='cnot', order_blocks: bool = True, efficient: bool = False) -> Circuit:
        ...
        Args:
            ...
            efficient: whether to use efficient ordering (default: False), i.e., without exploiting Clifford2Q cancellation
        """
        from phoenix_old.primitive import ordering, utils
        from phoenix_old.primitive.utils import optimize_phoenix_circuit_by_qiskit
