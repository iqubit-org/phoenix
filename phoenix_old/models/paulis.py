...
from phoenix_old.models.cliffords import Clifford1Q, Clifford2Q
from phoenix_old.utils.ops import params_u3
...
class BSF:
    ...
    #     """Concatenate two BSFs."""
    #     if not isinstance(other, BSF):
    #         raise TypeError("Can only add another BSF instance.")
    #     if self.num_qubits != other.num_qubits:
    #         raise ValueError("Cannot add BSFs with different number of qubits.")
    #
    #     new_bsf = BSF(np.vstack((self.mat, other.mat)),
    #                   np.hstack((self.coeffs, other.coeffs)),
    #                   np.hstack((self.signs, other.signs)))
    #     return new_bsf

    def concat(self, other: 'BSF') -> 'BSF':
        if not isinstance(other, BSF):
            raise TypeError("Can only add another BSF instance.")
        if self.num_qubits != other.num_qubits:
            raise ValueError("Cannot add BSFs with different number of qubits.")
        if other.num_paulis == 0:
            return self
    ...
    def print_tableau(self):
        ...
        for pauli, xz, sign in zip(self.paulis, self.mat, self.signs):
            x_part = xz[:self.num_qubits].astype(str)
            z_part = xz[self.num_qubits:].astype(str)
            table.add_row([pauli, ' '.join(x_part), ' '.join(z_part), sign])
    ...
    def total_weight(self) -> int:
        ...
        # return res.sum()
        return np.bitwise_or.reduce(matrices, axis=0).sum()
    ...
    @property
    def which_two_local_paulis(self):
        def is_isolated_two_local(lhs, multi_local_with_ops):
            if lhs.sum() == np.bitwise_and(lhs, multi_local_with_ops).sum(axis=1).max():
                return False
            return True

        with_ops = self.with_ops

        if len(self.which_multi_nonlocal_paulis) == 0:
            two_local_idx = np.where(self.with_ops.sum(axis=1) == 2)[0].tolist()
        else:
            two_local_idx = []
            multi_nonlocal_with_ops = self.with_ops[self.which_multi_nonlocal_paulis]
            for idx in np.where(self.with_ops.sum(axis=1) == 2)[0]:
                if is_isolated_two_local(self.mat[idx, :self.num_qubits] | self.mat[idx, self.num_qubits:],
                                         multi_nonlocal_with_ops):
                    two_local_idx.append(idx)

        # TODO: 这个sort函数需要类似group来考虑least_overlap
        two_local_idx = sorted(two_local_idx, key=lambda idx: with_ops[idx].tolist())
        return np.array(two_local_idx).astype(int)

    @property
    def which_multi_nonlocal_paulis(self) -> np.ndarray:
        """Which non-local Paulis have more than 2 qubits."""
        return np.where(self.with_ops.sum(axis=1) > 2)[0]
    ...
    def pop_two_local_paulis(self) -> 'BSF':
        popped = deepcopy(self)
        which_two_local_paulis = self.which_two_local_paulis
        popped.mat, popped.coeffs, popped.signs = popped.mat[which_two_local_paulis], popped.coeffs[
            which_two_local_paulis], popped.signs[which_two_local_paulis]
        mask = np.ones(self.num_paulis, dtype=bool)
        mask[which_two_local_paulis] = False
        self.mat, self.coeffs, self.signs = self.mat[mask], self.coeffs[mask], self.signs[mask]
        return popped

    def resort_two_local_paulis(self):
        # TODO: implement this
        ...
    ...
    def pop_local_paulis(self) -> 'BSF':
        ...
        """
        popped = deepcopy(self)
        which_local_paulis = self.which_local_paulis
        which_nonlocal_paulis = self.which_nonlocal_paulis
        popped.mat, popped.coeffs, popped.signs = popped.mat[which_local_paulis], popped.coeffs[
            which_local_paulis], popped.signs[which_local_paulis]
        self.mat, self.coeffs, self.signs = self.mat[which_nonlocal_paulis], self.coeffs[
            which_nonlocal_paulis], self.signs[which_nonlocal_paulis]
        return popped
    ...
    def as_su4_circuit(self) -> Circuit:
        ...
        from phoenix_old.primitive.utils import can_decompose
