...
from phoenix_old.utils.ops import replace_close_to_zero_with_zero
from phoenix_old.utils.functions import limit_angle
from phoenix_old.utils.graphs import draw_circ_dag_mpl, draw_circ_dag_graphviz, rx_to_nx_graph, node_index
...
class Circuit(list):
    ...
    def unitary(self, msb: bool = False, with_dummy: bool = False) -> np.ndarray:
        ...
        """
        from phoenix_old.utils.ops import tensor_slots, controlled_unitary_matrix, circuit_to_unitary
    ...
    def layer(self, on_body='dag') -> List[List[Gate]]:
        ...
        """
        if on_body == 'circuit':
            from phoenix_old.utils.passes import obtain_front_layer_from_circuit
            layers = []
            circ = self.clone()
            while circ:
            ...
            from phoenix_old.utils.passes import dag_to_layers
...
def parse_to_qasm_tuples(circuit: Circuit) -> List[Tuple[str, List[int]]]:
    ...
    for g in circuit:
        if not ((g.n_qubits == 1 and len(g.cqs) <= 1) or
                (len(g.tqs) == 2 and len(g.cqs) <= 1 and isinstance(g, (gates.SWAPGate, gates.ISWAPGate, gates.ISWAPDGGate))) or
        ...
        if isinstance(g, gates.IGate):
            ...
            gname = 'sx'
        elif isinstance(g, gates.VDGGate):
            gname = 'sxdg'
        else:
            gname = g.name.lower()
...
def _from_qiskit(qc: qiskit.QuantumCircuit) -> Circuit:
    ...
    for instr in qc.data:
        ...
        if opr.name == 'id':
            ...
            circ.append(gates.VDG.on(qubits))
        elif opr.name == 'h':
        ...
        elif opr.name == 'iswap_dg':
            circ.append(gates.ISWAPDG.on(qubits))
        elif opr.name == 'can':
