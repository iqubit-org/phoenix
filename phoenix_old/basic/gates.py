...
class Gate:
    ...
    def __init__(self, data: ndarray, name: str = None, *args, **kwargs):
        ...
        """
        from phoenix_old.utils.functions import is_power_of_two
...
class VDGGate(Gate):
    """VDGGate equals to the SXDGate in Qiskit"""

    def __init__(self, *args, **kwargs):
        super().__init__(np.array([[1 - 1j, 1 + 1j],
                                   [1 + 1j, 1 - 1j]]) / 2, name='VDG', *args, **kwargs)
    def hermitian(self):
        if self.tqs:
            return V.on(self.tqs, self.cqs)
        return VGate()
...
class ISWAPDGGate(Gate):
    def __init__(self, *args, **kwargs):
        super().__init__(np.array([[1. + 0.j, 0. + 0.j, 0. + 0.j, 0. + 0.j],
                                   [0. + 0.j, 0. + 0.j, -1.j, 0. + 0.j],
                                   [0. + 0.j, -1.j, 0. + 0.j, 0. + 0.j],
                                   [0. + 0.j, 0. + 0.j, 0. + 0.j, 1. + 0.j]]), name='ISWAPDG', *args, **kwargs)

    def hermitian(self):
        if self.tqs:
            return ISWAPGate.on(self.tqs, self.cqs)
        return ISWAPGate()
...
class Canonical(Gate):
    ...
    def weyl_coordinates(self) -> ndarray:
        ...
        """
        from phoenix_old.utils.functions import replace_close_to_zero_with_zero
