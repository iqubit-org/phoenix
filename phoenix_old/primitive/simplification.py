...
from phoenix_old.models.cliffords import Clifford2Q, CLIFFORD_2Q_SET
...
def search_cliffords(bsf: BSF, avoid: Tuple[int, int] = None) -> Tuple[BSF, float, Clifford2Q]:
    ...
    #     pairs_sorted = []
    #     while pairs:
    #         idx = least_overlap(pairs, pairs_sorted)
    #         pairs_sorted.append(idx)
    #         pairs.remove(idx)
    #     return pairs_sorted
...
def _heuristic_bsf_cost_func(bsf: BSF) -> float:
    ...
    if bsf.which_nonlocal_paulis.size > 1:
        ...
        cost += np.bitwise_or(bsf.with_ops[row_combs[0]], bsf.with_ops[row_combs[1]]).sum()
        cost += np.bitwise_or(bsf.x[row_combs[0]], bsf.x[row_combs[1]]).sum() * 0.5
        cost += np.bitwise_or(bsf.z[row_combs[0]], bsf.z[row_combs[1]]).sum() * 0.5
