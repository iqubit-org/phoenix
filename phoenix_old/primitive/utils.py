...
from phoenix_old.basic import gates, Gate
from phoenix_old.models.cliffords import Clifford2Q
from phoenix_old.models.paulis import BSF
from phoenix_old.utils.ops import params_zyz, params_u3, is_tensor_prod, kron_decomp, replace_close_to_zero_with_zero
...
two_qubit_cnot_decomposer = TwoQubitBasisDecomposer(CXGate())
...
def remove_identities(circuit: Circuit) -> Circuit:
    ...
    """
    from phoenix_old.utils.ops import is_equiv_unitary
...
def optimize_phoenix_circuit_by_qiskit(circ: Circuit, optimization_level=1) -> Circuit:
    """Topology-preserved optimization by Qiskit"""
    qc = circ.to_qiskit()

    from qiskit.transpiler import passes, PassManager
    from qiskit.transpiler import Target
    from qiskit.circuit.library import HGate, SGate, SdgGate, RZGate, U3Gate, CXGate
    from phoenix_old.ext.qiskit import CNOTEquivClifford
    inverse_list = []
    for p0, p1 in product(['X', 'Y', 'Z'], repeat=2):
        inverse_list.append(CNOTEquivClifford(p0, p1))
    pm = PassManager()
    pm.append(passes.InverseCancellation(inverse_list))
    pm.append(passes.CommutativeInverseCancellation(matrix_based=True))
    qc = pm.run(qc)

    # use qiskit.transpile to optimize the circuit with Target
    target = Target()
    # Add standard gates
    target.add_instruction(HGate())
    target.add_instruction(SGate())
    target.add_instruction(SdgGate())
    target.add_instruction(RZGate(0))
    target.add_instruction(U3Gate(0, 0, 0))
    target.add_instruction(CXGate())
    # Add custom Clifford2Q gates
    custom_name_mapping = {}
    for p0, p1 in product(['x', 'y', 'z'], repeat=2):
        gate_name = f'c{p0}{p1}'
        gate = CNOTEquivClifford(p0.upper(), p1.upper())
        custom_name_mapping[gate_name] = gate
        target.add_instruction(gate, name=gate_name)
    
    return Circuit.from_qiskit(qiskit.transpile(qc, optimization_level=optimization_level, target=target))


def config_to_circuit(config: List[Union[BSF, Clifford2Q]], by: str = 'cnot') -> Circuit:
    """
    ...
    for item in config:
        ...
        if isinstance(item, BSF):
            if by == 'cnot':
                # kernel = item.as_cnot_circuit()
                # if kernel.num_nonlocal_gates > 2: # TODO: if this item contains some local Paulis, this code corrupts
                #     print('kernal unitary shape', kernel.unitary().shape)
                #     qc = two_qubit_cnot_decomposer(Operator(kernel.unitary()).reverse_qargs())
                #     if qc.num_nonlocal_gates() <= 2:
                #         kernel = Circuit.from_qasm(qasm2.dumps(qc)).rewire({i: q for i, q in enumerate(kernel.qubits)})
