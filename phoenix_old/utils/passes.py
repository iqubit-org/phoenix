...
from phoenix_old.basic.gates import Gate
from phoenix_old.basic.circuits import Circuit
from phoenix_old.utils.graphs import find_predecessors_by_node, find_successors_by_node, node_index
...
def remove_front_cliffords(circ: Circuit) -> Circuit:
    """
    Remove all front Clifford gates from the circuit.
    """
    from phoenix_old.basic.gates import Clifford2QGate
    # circ = circ.clone()
    dag = circ.to_dag()
    while obtain_front_layer(dag, predicate=lambda g: isinstance(g, Clifford2QGate)):
        front_cliffs, indices = obtain_front_layer(dag, predicate=lambda g: isinstance(g, Clifford2QGate), return_indices=True)
        dag.remove_nodes_from(indices)

    return dag_to_circuit(dag)
        

def remove_last_cliffords(circ: Circuit) -> Circuit:
    """
    Remove all front Clifford gates from the circuit.
    """
    from phoenix_old.basic.gates import Clifford2QGate
    ...
    dag = circ.to_dag()
    predicate = lambda g: isinstance(g, (gates.XGate, gates.YGate, gates.ZGate, gates.HGate, gates.SGate, gates.VGate, gates.SDGGate, gates.VDGGate, gates.SWAPGate))
    while obtain_last_layer(dag, predicate=predicate):
        front_cliffs, indices = obtain_last_layer(dag, predicate=predicate, return_indices=True)
        dag.remove_nodes_from(indices)
