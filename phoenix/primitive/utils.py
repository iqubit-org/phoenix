...
def optimize_phoenix_circuit_by_qiskit(circ: Circuit, optimization_level=1) -> Circuit:
    ...
    from phoenix.ext.qiskit import Clifford2QGate as QiskitClifford2QGate
    inverse_list = []
    for p0, p1 in product(['X', 'Y', 'Z'], repeat=2):
        inverse_list.append(QiskitClifford2QGate(p0, p1))
    pm = PassManager()
    pm.append(passes.InverseCancellation(inverse_list))
    pm.append(passes.CommutativeInverseCancellation(matrix_based=True))
    qc = pm.run(qc)

    # use qiskit.transpile to optimize the circuit
    basis_gates = ['h', 's', 'sdg', 'rz', 'u3', 'cx']
    for p0, p1 in product(['x', 'y', 'z'], repeat=2):
        basis_gates.append(f'c{p0}{p1}')
    return Circuit.from_qiskit(qiskit.transpile(qc, optimization_level=optimization_level, basis_gates=basis_gates))
