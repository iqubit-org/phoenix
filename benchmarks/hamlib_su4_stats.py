...
stats = {"binaryoptimization": {}, "discreteoptimization": {}, "chemistry": {}, "condensedmatter": {}}
for dir in stats:
    ...
    for fname in tqdm(natsorted(os.listdir(os.path.join("./hamlib_qasm/", dir)))):
        ...
        circ_su4 = Circuit(
            [
                gates.X.on(blk.location[0], blk.location[1])
                for blk in blocks
                if not is_tensor_prod(blk.get_unitary().numpy)
            ]
        )
