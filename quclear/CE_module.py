...
def find_single_tree(base_entangler_inv: str, match_entangler_inv: str):
    ...
    roots_dict = {"Z_root": Z_root, "I_root": I_root, "Y_root": Y_root, "X_root": X_root, "N_root": N_root}
...
# #TODO: these is a special case for the base entangler only contains one non identity string.
def find_leaves(
    sorted_entanglers_params_inv: List[List[str]],
    curr_pauli,
    updated_paulis,
    qc_tree,
    tree_list,
...
def calculate_opt_weight_fast(base_entangler_inv: str, match_entangler_inv: str) -> int:
    ...
    optimized_paulis = update_paulis(
        [match_entangler_inv], extracted_cx_tree, parameters=False
    )  # construct_opt_pauli(entangler = match_entangler_inv, clifford_circuit = extracted_cx_tree)
...
def find_best_pauli_index_threshold(
    base_entangler: str, commute_sets: List[List[str]], append_clifford, threshold=1
) -> int:
    ...
    for idx, entangler in enumerate(ordered_entanglers):
        updated_entanglers = update_paulis(
            Paulis_params_list=[entangler], clifford_circuit=append_clifford, parameters=True
        )
...
def CE_recur_tree_seq(entanglers: List[str], params: List[float], barrier=False, threshold=1):
    ...
    for commute_idx, sorted_list in enumerate(sorted_entanglers_params):
        ...
        for pauli_idx in range(len(sorted_list)):
            ...
            if (
                pauli_idx == len(sorted_list) - 1
            ):  # if this pauli is the last one in a commuting list, find the pauli in the next list
                ...
                next_pauli_index = find_best_pauli_index_threshold(
                    base_entangler=curr_pauli,
                    commute_sets=sorted_entanglers_params[commute_idx][pauli_idx + 1 :],
                    append_clifford=append_clifford,
                    threshold=threshold,
                )
            ...
            if next_pauli == None:
                ...
                sq_index = find_leaves(
                    sorted_entanglers_params,
                    curr_pauli=curr_pauli,
                    updated_paulis={},
                    qc_tree=init_cx_tree,
                    tree_list=tree_list,
                    commute_idx=next_commute_idx,
                    pauli_idx=next_pauli_idx,
                    append_clifford=append_clifford,
                )
                ...
                if pauli_idx == len(sorted_list) - 1:
                    ...
                    sorted_entanglers_params[commute_idx + 1][0] = update_pauli_param(
                        Pauli_param=sorted_entanglers_params[commute_idx + 1][0], clifford_circuit=append_clifford
                    )
                else:
