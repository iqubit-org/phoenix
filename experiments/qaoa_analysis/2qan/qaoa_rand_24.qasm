OPENQASM 2.0;
include "qelib1.inc";
qreg q[64];
creg c[24];
u2(0,pi) q[0];
u2(0,pi) q[1];
u2(0,pi) q[2];
u2(0,pi) q[3];
u2(0,pi) q[4];
u2(0,pi) q[5];
cx q[4],q[5];
cx q[5],q[4];
cx q[4],q[5];
cx q[3],q[4];
cx q[4],q[3];
cx q[3],q[4];
u3(0.0013232938,-pi/2,0) q[3];
u2(pi/2,-pi/2) q[5];
u2(0,pi) q[6];
u2(0,pi) q[7];
u2(0,pi) q[8];
u2(0,pi) q[9];
u2(0,pi) q[10];
cx q[0],q[10];
cx q[10],q[0];
cx q[0],q[10];
cx q[0],q[1];
u1(0.1) q[1];
cx q[0],q[1];
cx q[1],q[2];
cx q[2],q[1];
cx q[1],q[2];
cx q[0],q[1];
u1(0.1) q[1];
cx q[0],q[1];
u2(pi/2,-pi/2) q[2];
cx q[2],q[3];
u2(-pi/2,pi/2) q[2];
u3(0.0018714197,-3*pi/4,-3*pi/4) q[3];
cx q[2],q[3];
u3(1.4707963,-pi,pi/2) q[2];
u3(1.569473,1.569473,-pi) q[3];
cx q[2],q[3];
u2(-pi,pi/2) q[2];
cx q[1],q[2];
cx q[2],q[1];
cx q[1],q[2];
u3(0.0013232938,-pi/2,0) q[1];
u2(1.569473,-pi) q[3];
cx q[3],q[4];
cx q[4],q[3];
cx q[3],q[4];
u2(0,pi) q[11];
cx q[4],q[11];
u1(0.1) q[11];
cx q[4],q[11];
u2(0,pi) q[12];
cx q[8],q[12];
cx q[12],q[8];
cx q[8],q[12];
cx q[7],q[8];
cx q[8],q[7];
cx q[7],q[8];
cx q[6],q[7];
cx q[7],q[6];
cx q[6],q[7];
u3(0.0013232938,-pi/2,0) q[6];
cx q[5],q[6];
u2(-pi/2,pi/2) q[5];
u3(0.0018714197,-3*pi/4,-3*pi/4) q[6];
cx q[5],q[6];
u3(1.4707963,-pi,pi/2) q[5];
u3(1.569473,1.569473,-pi) q[6];
cx q[5],q[6];
u3(1.569473,pi/2,pi/2) q[5];
u2(1.569473,-pi) q[6];
cx q[6],q[7];
cx q[7],q[6];
cx q[6],q[7];
cx q[8],q[12];
cx q[12],q[8];
cx q[8],q[12];
cx q[8],q[9];
cx q[9],q[8];
cx q[8],q[9];
cx q[7],q[8];
cx q[8],q[7];
cx q[7],q[8];
u2(pi/2,-pi/2) q[8];
u3(0.0013232938,-pi/2,0) q[9];
u2(0,pi) q[13];
cx q[10],q[13];
u1(0.1) q[13];
cx q[10],q[13];
u2(pi/2,-pi/2) q[13];
u2(0,pi) q[14];
u2(0,pi) q[15];
cx q[14],q[15];
cx q[15],q[14];
cx q[14],q[15];
u3(0.0013232938,-pi/2,0) q[14];
cx q[13],q[14];
u2(-pi/2,pi/2) q[13];
u3(0.0018714197,-3*pi/4,-3*pi/4) q[14];
cx q[13],q[14];
u3(1.4707963,-pi,pi/2) q[13];
u3(1.569473,1.569473,-pi) q[14];
cx q[13],q[14];
u2(-pi,pi/2) q[13];
cx q[10],q[13];
u1(0.1) q[13];
cx q[10],q[13];
cx q[0],q[10];
cx q[10],q[0];
cx q[0],q[10];
u2(pi/2,-pi/2) q[0];
cx q[0],q[1];
u2(-pi/2,pi/2) q[0];
u3(0.0018714197,-3*pi/4,-3*pi/4) q[1];
cx q[0],q[1];
u3(1.4707963,-pi,pi/2) q[0];
u3(1.569473,1.569473,-pi) q[1];
cx q[0],q[1];
u2(-pi,pi/2) q[0];
u2(1.569473,-pi) q[1];
cx q[1],q[2];
u2(pi/2,-pi/2) q[10];
u2(1.569473,-pi) q[14];
cx q[2],q[1];
cx q[1],q[2];
u2(pi/2,-pi/2) q[1];
cx q[2],q[3];
cx q[3],q[2];
cx q[2],q[3];
u3(0.0013232938,-pi/2,0) q[2];
cx q[1],q[2];
u2(-pi/2,pi/2) q[1];
u3(0.0018714197,-3*pi/4,-3*pi/4) q[2];
cx q[1],q[2];
u3(1.4707963,-pi,pi/2) q[1];
u3(1.569473,1.569473,-pi) q[2];
cx q[1],q[2];
u2(-pi,pi/2) q[1];
cx q[0],q[1];
cx q[1],q[0];
cx q[0],q[1];
u3(0.0013232938,-pi/2,0) q[1];
u2(1.569473,-pi) q[2];
cx q[3],q[4];
cx q[4],q[3];
cx q[3],q[4];
u2(pi/2,-pi/2) q[3];
u2(pi/2,-pi/2) q[4];
cx q[4],q[5];
u2(-pi/2,pi/2) q[4];
u3(0.0018714197,-3*pi/4,-3*pi/4) q[5];
cx q[4],q[5];
u3(1.4707963,-pi,pi/2) q[4];
u3(1.569473,1.569473,-pi) q[5];
cx q[4],q[5];
u3(1.569473,pi/2,pi/2) q[4];
cx q[3],q[4];
u2(-pi/2,pi/2) q[3];
u3(0.0018714197,-3*pi/4,-3*pi/4) q[4];
cx q[3],q[4];
u3(1.4707963,-pi,pi/2) q[3];
u3(1.569473,1.569473,-pi) q[4];
cx q[3],q[4];
u2(-pi,pi/2) q[3];
cx q[2],q[3];
cx q[3],q[2];
cx q[2],q[3];
u2(1.569473,-pi) q[4];
cx q[3],q[4];
cx q[4],q[3];
cx q[3],q[4];
u2(1.569473,-pi) q[5];
u2(0,pi) q[16];
cx q[15],q[16];
cx q[16],q[15];
cx q[15],q[16];
cx q[14],q[15];
cx q[15],q[14];
cx q[14],q[15];
cx q[13],q[14];
u1(0.1) q[14];
cx q[13],q[14];
u3(0.0013232938,-pi/2,0) q[13];
cx q[10],q[13];
u2(-pi/2,pi/2) q[10];
u3(0.0018714197,-3*pi/4,-3*pi/4) q[13];
cx q[10],q[13];
u3(1.4707963,-pi,pi/2) q[10];
u3(1.569473,1.569473,-pi) q[13];
cx q[10],q[13];
u2(-pi,pi/2) q[10];
cx q[0],q[10];
cx q[10],q[0];
cx q[0],q[10];
u2(pi/2,-pi/2) q[0];
cx q[0],q[1];
u2(-pi/2,pi/2) q[0];
u3(0.0018714197,-3*pi/4,-3*pi/4) q[1];
cx q[0],q[1];
u3(1.4707963,-pi,pi/2) q[0];
u3(1.569473,1.569473,-pi) q[1];
cx q[0],q[1];
u2(-pi,pi/2) q[0];
u2(1.569473,-pi) q[1];
u2(pi/2,-pi/2) q[10];
u2(1.569473,-pi) q[13];
u2(pi/2,-pi/2) q[15];
u2(pi/2,-pi/2) q[16];
u3(1.5721196,-pi/2,-pi) q[17];
cx q[16],q[17];
u2(-pi/2,pi/2) q[16];
u3(0.0018714197,-3*pi/4,-3*pi/4) q[17];
cx q[16],q[17];
u3(1.4707963,-pi,pi/2) q[16];
u3(1.569473,1.569473,-pi) q[17];
cx q[16],q[17];
u2(-pi,pi/2) q[16];
u2(1.569473,-pi) q[17];
cx q[11],q[17];
cx q[17],q[11];
cx q[11],q[17];
u2(0,pi) q[18];
cx q[17],q[18];
cx q[18],q[17];
cx q[17],q[18];
cx q[16],q[17];
cx q[17],q[16];
cx q[16],q[17];
u3(0.0013232938,-pi/2,0) q[16];
cx q[15],q[16];
u2(-pi/2,pi/2) q[15];
u3(0.0018714197,-3*pi/4,-3*pi/4) q[16];
cx q[15],q[16];
u3(1.4707963,-pi,pi/2) q[15];
u3(1.569473,1.569473,-pi) q[16];
cx q[15],q[16];
u2(-pi,pi/2) q[15];
cx q[14],q[15];
cx q[15],q[14];
cx q[14],q[15];
cx q[13],q[14];
u1(0.1) q[14];
cx q[13],q[14];
u2(1.569473,-pi) q[16];
u2(0,pi) q[19];
u2(0,pi) q[20];
cx q[19],q[20];
u1(0.1) q[20];
cx q[19],q[20];
cx q[18],q[19];
cx q[19],q[18];
cx q[18],q[19];
cx q[17],q[18];
cx q[18],q[17];
cx q[17],q[18];
cx q[16],q[17];
cx q[17],q[16];
cx q[16],q[17];
cx q[15],q[16];
u1(0.1) q[16];
cx q[15],q[16];
cx q[14],q[15];
cx q[15],q[14];
cx q[14],q[15];
cx q[13],q[14];
u1(0.1) q[14];
cx q[13],q[14];
u3(0.0013232938,-pi/2,0) q[13];
cx q[10],q[13];
u2(-pi/2,pi/2) q[10];
u3(0.0018714197,-3*pi/4,-3*pi/4) q[13];
cx q[10],q[13];
u3(1.4707963,-pi,pi/2) q[10];
u3(1.569473,1.569473,-pi) q[13];
cx q[10],q[13];
u2(-pi,pi/2) q[10];
u2(1.569473,-pi) q[13];
u2(pi/2,-pi/2) q[19];
u3(0.0013232938,-pi/2,0) q[20];
cx q[19],q[20];
u2(-pi/2,pi/2) q[19];
u3(0.0018714197,-3*pi/4,-3*pi/4) q[20];
cx q[19],q[20];
u3(1.4707963,-pi,pi/2) q[19];
u3(1.569473,1.569473,-pi) q[20];
cx q[19],q[20];
u2(-pi,pi/2) q[19];
cx q[18],q[19];
u1(0.1) q[19];
cx q[18],q[19];
cx q[17],q[18];
cx q[18],q[17];
cx q[17],q[18];
cx q[16],q[17];
cx q[17],q[16];
cx q[16],q[17];
cx q[15],q[16];
cx q[16],q[15];
cx q[15],q[16];
u2(pi/2,-pi/2) q[18];
u2(1.569473,-pi) q[20];
u2(0,pi) q[21];
cx q[20],q[21];
cx q[21],q[20];
cx q[20],q[21];
cx q[12],q[21];
cx q[19],q[20];
cx q[20],q[19];
cx q[19],q[20];
u3(0.0013232938,-pi/2,0) q[19];
cx q[18],q[19];
u2(-pi/2,pi/2) q[18];
u3(0.0018714197,-3*pi/4,-3*pi/4) q[19];
cx q[18],q[19];
u3(1.4707963,-pi,pi/2) q[18];
u3(1.569473,1.569473,-pi) q[19];
cx q[18],q[19];
u2(-pi,pi/2) q[18];
u2(1.569473,-pi) q[19];
u2(pi/2,-pi/2) q[20];
cx q[21],q[12];
cx q[12],q[21];
u3(0.0013232938,-pi/2,0) q[12];
u3(0.0013232938,-pi/2,0) q[21];
cx q[20],q[21];
u2(-pi/2,pi/2) q[20];
u3(0.0018714197,-3*pi/4,-3*pi/4) q[21];
cx q[20],q[21];
u3(1.4707963,-pi,pi/2) q[20];
u3(1.569473,1.569473,-pi) q[21];
cx q[20],q[21];
u2(-pi,pi/2) q[20];
cx q[19],q[20];
cx q[20],q[19];
cx q[19],q[20];
cx q[18],q[19];
cx q[19],q[18];
cx q[18],q[19];
cx q[17],q[18];
u1(0.1) q[18];
cx q[17],q[18];
cx q[11],q[17];
cx q[17],q[11];
cx q[11],q[17];
cx q[17],q[18];
u1(0.1) q[18];
cx q[17],q[18];
u2(1.569473,-pi) q[21];
cx q[4],q[11];
cx q[11],q[4];
cx q[4],q[11];
cx q[11],q[17];
cx q[17],q[11];
cx q[11],q[17];
cx q[16],q[17];
cx q[17],q[16];
cx q[16],q[17];
cx q[4],q[5];
u1(0.1) q[5];
cx q[4],q[5];
cx q[4],q[11];
cx q[11],q[4];
cx q[4],q[11];
cx q[3],q[4];
u1(0.1) q[4];
cx q[3],q[4];
cx q[5],q[6];
cx q[6],q[5];
cx q[5],q[6];
cx q[4],q[5];
cx q[5],q[4];
cx q[4],q[5];
cx q[4],q[11];
cx q[11],q[4];
cx q[4],q[11];
cx q[11],q[17];
u1(0.1) q[17];
cx q[11],q[17];
cx q[17],q[18];
cx q[18],q[17];
cx q[17],q[18];
cx q[11],q[17];
u1(0.1) q[17];
cx q[11],q[17];
cx q[18],q[19];
cx q[19],q[18];
cx q[18],q[19];
u2(pi/2,-pi/2) q[18];
cx q[19],q[20];
cx q[20],q[19];
cx q[19],q[20];
u2(pi/2,-pi/2) q[19];
cx q[8],q[12];
u3(0.0018714197,-3*pi/4,-3*pi/4) q[12];
u2(-pi/2,pi/2) q[8];
cx q[8],q[12];
u3(1.569473,1.569473,-pi) q[12];
u3(1.4707963,-pi,pi/2) q[8];
cx q[8],q[12];
u2(1.569473,-pi) q[12];
u2(-pi,pi/2) q[8];
cx q[7],q[8];
u1(0.1) q[8];
cx q[7],q[8];
u2(pi/2,-pi/2) q[8];
cx q[8],q[9];
u2(-pi/2,pi/2) q[8];
u3(0.0018714197,-3*pi/4,-3*pi/4) q[9];
cx q[8],q[9];
u3(1.4707963,-pi,pi/2) q[8];
u3(1.569473,1.569473,-pi) q[9];
cx q[8],q[9];
u2(-pi,pi/2) q[8];
u2(1.569473,-pi) q[9];
u2(0,pi) q[22];
cx q[21],q[22];
cx q[22],q[21];
cx q[21],q[22];
cx q[12],q[21];
u1(0.1) q[21];
cx q[12],q[21];
cx q[20],q[21];
cx q[21],q[20];
cx q[20],q[21];
u3(0.0013232938,-pi/2,0) q[20];
cx q[19],q[20];
u2(-pi/2,pi/2) q[19];
u3(0.0018714197,-3*pi/4,-3*pi/4) q[20];
cx q[19],q[20];
u3(1.4707963,-pi,pi/2) q[19];
u3(1.569473,1.569473,-pi) q[20];
cx q[19],q[20];
u3(1.569473,pi/2,pi/2) q[19];
cx q[18],q[19];
u2(-pi/2,pi/2) q[18];
u3(0.0018714197,-3*pi/4,-3*pi/4) q[19];
cx q[18],q[19];
u3(1.4707963,-pi,pi/2) q[18];
u3(1.569473,1.569473,-pi) q[19];
cx q[18],q[19];
u2(-pi,pi/2) q[18];
cx q[17],q[18];
cx q[18],q[17];
cx q[17],q[18];
cx q[16],q[17];
cx q[17],q[16];
cx q[16],q[17];
cx q[15],q[16];
cx q[16],q[15];
cx q[15],q[16];
cx q[14],q[15];
cx q[15],q[14];
cx q[14],q[15];
u2(pi/2,-pi/2) q[15];
cx q[16],q[17];
u1(0.1) q[17];
cx q[16],q[17];
u3(0.0013232938,-pi/2,0) q[16];
cx q[15],q[16];
u2(-pi/2,pi/2) q[15];
u3(0.0018714197,-3*pi/4,-3*pi/4) q[16];
cx q[15],q[16];
u3(1.4707963,-pi,pi/2) q[15];
u3(1.569473,1.569473,-pi) q[16];
cx q[15],q[16];
u2(-pi,pi/2) q[15];
u2(1.569473,-pi) q[16];
u2(1.569473,-pi) q[19];
u2(1.569473,-pi) q[20];
u2(pi/2,-pi/2) q[22];
u3(1.5721196,-pi/2,-pi) q[23];
cx q[22],q[23];
u2(-pi/2,pi/2) q[22];
u3(0.0018714197,-3*pi/4,-3*pi/4) q[23];
cx q[22],q[23];
u3(1.4707963,-pi,pi/2) q[22];
u3(1.569473,1.569473,-pi) q[23];
cx q[22],q[23];
u2(-pi,pi/2) q[22];
u2(1.569473,-pi) q[23];
measure q[6] -> c[0];
measure q[3] -> c[1];
measure q[17] -> c[2];
measure q[13] -> c[3];
measure q[12] -> c[4];
measure q[0] -> c[5];
measure q[11] -> c[6];
measure q[18] -> c[7];
measure q[8] -> c[8];
measure q[7] -> c[9];
measure q[10] -> c[10];
measure q[9] -> c[11];
measure q[2] -> c[12];
measure q[20] -> c[13];
measure q[5] -> c[14];
measure q[1] -> c[15];
measure q[16] -> c[16];
measure q[15] -> c[17];
measure q[21] -> c[18];
measure q[4] -> c[19];
measure q[23] -> c[20];
measure q[19] -> c[21];
measure q[14] -> c[22];
measure q[22] -> c[23];
