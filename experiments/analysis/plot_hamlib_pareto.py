#!/usr/bin/env python
"""Pareto scatter of Symphony vs SOTA on HamLib (100 programs, all-to-all).
...
Symphony (our holistic engine, CSV key ``phoenixpp``) is the sole point in the
...
OUT = os.path.join(PARENT_DIR, "figures", "hamlib_pareto.pdf")
...
def main():
    ...
    # region Pareto-dominated by Symphony: worse (higher) on both axes
    ...
    # baselines (recessive field) then Symphony (hero) on top
    ...
    # chrome
    ax.set_xlabel("2-qubit depth  ·  optimization rate",
                  color=INK, fontsize=11.5)
    ax.set_ylabel("2-qubit gate count  ·  optimization rate",
                  color=INK, fontsize=11.5)
    # ax.set_title("Symphony Pareto-dominates SOTA on HamLib", color=INK, fontsize=13.5, fontweight="bold", pad=12)
    ...
    handles = [
        Line2D([0], [0], marker="*", color="none", markerfacecolor=ACCENT,
               markeredgecolor=ACCENT, markersize=16, label="Symphony (ours)"),
    ...
    print(f"saved -> {os.path.relpath(OUT, PARENT_DIR)}")
