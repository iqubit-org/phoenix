...
# generated by `make naive -f Makefile-Hamlib` — replaces the retired
# benchmarks/hamlib_qasm dir.
BENCHMARK_DPATH = "./output_hamlib/naive"
...
def main():
    ...
    if not os.path.isdir(BENCHMARK_DPATH) or not any(
        fn.endswith(".qasm")
        for root, _, fns in os.walk(BENCHMARK_DPATH)
        for fn in fns
    ):
        raise ValueError(
            "{} is missing or empty — run `make naive -f Makefile-Hamlib` first "
            "to generate the original-circuit reference (note: it is empty "
            "after an old-style `make clean`)".format(BENCHMARK_DPATH)
        )
    ...
    tasks = []
    for category in sorted(os.listdir(BENCHMARK_DPATH)):
        if not os.path.isdir(os.path.join(BENCHMARK_DPATH, category)):
            continue  # stray files (e.g. .DS_Store)
